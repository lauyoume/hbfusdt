...
func (p *OrderClient) PlaceOrder(request *order.PlaceOrderRequest) (*order.PlaceOrderResponse, error) {
	...
	url := p.privateUrlBuilder.Build("POST", "/linear-swap-api/v1/swap_order", nil)
	postResp, postErr := internal.HttpPost(url, postBody)
	if postErr != nil {
		return nil, postErr
	}

	fmt.Println(string(postResp), string(postBody))
...
func (p *OrderClient) PlaceOrders(request []order.PlaceOrderRequest) (*order.PlaceOrdersResponse, error) {
	...
	reqdata := order.PlaceOrderBatchRequest{
		OrdersData: request,
	}
	postBody, jsonErr := model.ToJson(reqdata)

	url := p.privateUrlBuilder.Build("POST", "/linear-swap-api/v1/swap_batchorder", nil)
...
// Cancel order
func (p *OrderClient) CancelOrder(request *order.CancelOrderRequest) (*order.CancelOrderResponse, error) {
	...
	result := order.CancelOrderResponse{}
	jsonErr = json.Unmarshal([]byte(postResp), &result)
	if jsonErr != nil {
		return nil, jsonErr
	}
